...
import AdminCrecheDetail from './AdminCrecheDetail';
...
export default function AdminCenter({ onNavigate }: AdminCenterProps) {
  ...
  const platformAdmins = useMemo(() => comptes.filter(account => account.role === 'admin'), [comptes]);
  ...
  const overdueFollowups = useMemo(() => adminFollowups.filter(item => item.status === 'planned' && item.dueAt && item.dueAt <= todayKey), [adminFollowups, todayKey]);
  const unassignedTickets = useMemo(() => signalements.filter(item => item.statut !== 'resolu' && item.statut !== 'rejete' && !item.assigneeId), [signalements]);
  const dueTickets = useMemo(() => signalements.filter(item => item.statut !== 'resolu' && item.statut !== 'rejete' && item.dueAt && item.dueAt <= todayKey), [signalements, todayKey]);
  ...
  const selectedDirector = selectedDirectorId ? directors.find(account => account.id === selectedDirectorId) || null : null;
  const activeRate = directors.length ? Math.round((directors.filter(account => account.abonnementActif).length / directors.length) * 100) : 0;
  const filteredAuditLogs = adminAuditLogs.filter(entry => {
    const term = historySearch.trim().toLocaleLowerCase();
    const matchesTerm = !term || [entry.targetLabel, entry.targetId, entry.action].filter(Boolean).join(' ').toLocaleLowerCase().includes(term);
    return matchesTerm && (historyAction === 'all' || entry.action === historyAction);
  });
  ...
  const exportCrèches = () => downloadCsv('rawdha-plus-creches.csv', [text('Crèche', 'الروضة'), text('Directrice', 'المديرة'), 'Email', text('Ville', 'المدينة'), text('Abonnement', 'الاشتراك'), text('Étape commerciale', 'المرحلة التجارية'), text('Échéance', 'الانتهاء'), text('Prochaine relance', 'المتابعة القادمة'), text('Dernière activité', 'آخر نشاط')], filteredDirectors.map(account => [accountLabel(account), `${account.prenom} ${account.nom}`, account.email, account.ville, account.abonnementActif ? text('Actif', 'نشط') : text('Suspendu', 'معلق'), account.commercialStage || (account.abonnementActif ? text('Abonné', 'مشترك') : text('Nouveau', 'جديد')), account.dateFinAbonnement, account.nextFollowUpAt, account.lastActivityAt]));
  const exportHistorique = () => downloadCsv('rawdha-plus-historique-admin.csv', [text('Date', 'التاريخ'), text('Action', 'الإجراء'), text('Cible', 'الهدف'), text('Type', 'النوع')], filteredAuditLogs.map(entry => [entry.createdAt, entry.action, entry.targetLabel || entry.targetId, entry.targetType]));
  ...
  return (
    <div className="min-w-0 space-y-5 sm:space-y-7" dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      {view === 'pilotage' && <>
        ...
        <section className="grid gap-5 xl:grid-cols-[minmax(0,1.18fr)_minmax(20rem,.82fr)]">
          <div className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm sm:p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-black text-slate-950">{text('Priorités du jour', 'أولويات اليوم')}</h2><p className="mt-1 text-xs text-slate-500">{text('Ouvrez chaque file pour agir directement.', 'افتحوا كل قائمة للتصرف مباشرة.')}</p></div><AlertTriangle className="h-5 w-5 text-amber-500" /></div><div className="mt-4 space-y-2.5">
            ...
            {renderPriority(<Clock3 className="h-5 w-5" />, text('Relances à effectuer', 'متابعات يجب تنفيذها'), text('Relances planifiées arrivées à échéance', 'متابعات مخططة وصلت إلى موعدها'), overdueFollowups.length, text('Voir les crèches', 'عرض الروضات'), () => setView('creches'), 'indigo')}
          ...
          <div className="rounded-3xl border border-slate-100 bg-slate-950 p-5 text-white shadow-lg shadow-slate-900/10"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-indigo-200">{text('État de la plateforme', 'حالة المنصة')}</p><h2 className="mt-2 text-xl font-black">{text('Gardez une vue nette, sans multiplier les écrans.', 'تابعوا المنصة بوضوح دون التنقل بين شاشات كثيرة.')}</h2><div className="mt-5 space-y-3"><div className="flex items-center justify-between rounded-2xl bg-white/10 px-3 py-3"><span className="text-xs font-bold text-slate-200">{text('Taux d’abonnements actifs', 'نسبة الاشتراكات النشطة')}</span><span className="text-lg font-black">{activeRate}%</span></div><div className="flex items-center justify-between rounded-2xl bg-white/10 px-3 py-3"><span className="text-xs font-bold text-slate-200">{text('Directrices inactives 14j+', 'مديرات غير نشطات 14+ يوماً')}</span><span className="text-lg font-black">{inactiveDirectors.length}</span></div><div className="flex items-center justify-between rounded-2xl bg-white/10 px-3 py-3"><span className="text-xs font-bold text-slate-200">{text('Profils à compléter', 'ملفات تحتاج إكمالاً')}</span><span className="text-lg font-black">{incompleteDirectors.length}</span></div><div className="flex items-center justify-between rounded-2xl bg-white/10 px-3 py-3"><span className="text-xs font-bold text-slate-200">{text('Abonnements suspendus', 'اشتراكات معلقة')}</span><span className="text-lg font-black">{suspendedDirectors.length}</span></div></div><button type="button" onClick={() => setView('qualite')} className="mt-5 inline-flex items-center gap-2 text-xs font-black text-white underline decoration-indigo-300 underline-offset-4">{text('Voir le contrôle qualité', 'عرض مراقبة الجودة')}<ArrowRight className="h-4 w-4 rtl:rotate-180" /></button></div>
        </section>
        <section className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-rose-600">{text('Alertes opérationnelles', 'تنبيهات تشغيلية')}</p><h2 className="mt-1 text-lg font-black text-slate-950">{text('Les dossiers à reprendre en priorité', 'الملفات ذات الأولوية')}</h2><p className="mt-1 text-xs text-slate-500">{text('Ces alertes viennent des relances et tickets réellement enregistrés.', 'هذه التنبيهات تأتي من المتابعات والطلبات المسجلة فعلياً.')}</p></div><span className="rounded-full bg-rose-50 px-3 py-1.5 text-xs font-black text-rose-700">{overdueFollowups.length + unassignedTickets.length + dueTickets.length} {text('signalement(s)', 'تنبيه')}</span></div><div className="mt-4 grid gap-3 lg:grid-cols-3"><button type="button" onClick={() => setView('creches')} className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-3 text-start transition hover:bg-indigo-50"><p className="text-xs font-black text-indigo-900">{text('Relances arrivées à échéance', 'متابعات وصلت إلى موعدها')}</p><p className="mt-2 text-2xl font-black text-indigo-700">{overdueFollowups.length}</p><p className="mt-1 text-[11px] font-semibold text-indigo-700/70">{text('À retrouver dans la fiche crèche', 'موجودة في ملف الروضة')}</p></button><button type="button" onClick={() => onNavigate('communication')} className="rounded-2xl border border-amber-100 bg-amber-50/60 p-3 text-start transition hover:bg-amber-50"><p className="text-xs font-black text-amber-900">{text('Tickets non attribués', 'طلبات غير معينة')}</p><p className="mt-2 text-2xl font-black text-amber-700">{unassignedTickets.length}</p><p className="mt-1 text-[11px] font-semibold text-amber-700/70">{text('Attribuez un responsable dans Support', 'عيّنوا مسؤولاً في الدعم')}</p></button><button type="button" onClick={() => onNavigate('communication')} className="rounded-2xl border border-rose-100 bg-rose-50/60 p-3 text-start transition hover:bg-rose-50"><p className="text-xs font-black text-rose-900">{text('Tickets à échéance', 'طلبات وصلت إلى موعدها')}</p><p className="mt-2 text-2xl font-black text-rose-700">{dueTickets.length}</p><p className="mt-1 text-[11px] font-semibold text-rose-700/70">{text('À traiter ou reprogrammer', 'للمعالجة أو إعادة الجدولة')}</p></button></div></section>
        <section className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-600">{text('Équipe interne', 'الفريق الداخلي')}</p><h2 className="mt-1 text-lg font-black text-slate-950">{text('Délégation préparée, accès inchangés', 'تفويض مُجهز، صلاحيات دون تغيير')}</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">{text('Les profils Support et Commercial sont prêts dans le modèle d’organisation. Ils ne reçoivent aucun accès à la plateforme tant qu’une délégation de sécurité dédiée n’est pas créée.', 'تم تجهيز ملفات الدعم والتجاري في نموذج التنظيم. لا يحصلون على أي صلاحية في المنصة قبل إنشاء تفويض أمني خاص.')}</p></div><ShieldCheck className="h-6 w-6 shrink-0 text-indigo-500" /></div><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-slate-950 p-3 text-white"><p className="text-[10px] font-black uppercase tracking-wide text-indigo-200">Super Admin</p><p className="mt-2 text-lg font-black">{platformAdmins.length}</p><p className="mt-1 text-[11px] text-slate-300">{text('Accès complet actuel', 'صلاحية كاملة حالياً')}</p></div><div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-3"><p className="text-[10px] font-black uppercase tracking-wide text-emerald-700">Support</p><p className="mt-2 text-sm font-black text-emerald-900">{text('Prêt à déléguer', 'جاهز للتفويض')}</p><p className="mt-1 text-[11px] text-emerald-800">{text('Tickets et réponses, sans abonnements', 'طلبات وردود، دون اشتراكات')}</p></div><div className="rounded-2xl border border-amber-100 bg-amber-50 p-3"><p className="text-[10px] font-black uppercase tracking-wide text-amber-700">Commercial</p><p className="mt-2 text-sm font-black text-amber-900">{text('Prêt à déléguer', 'جاهز للتفويض')}</p><p className="mt-1 text-[11px] text-amber-800">{text('Pipeline et relances, sans données sensibles', 'متابعة ومكالمات، دون بيانات حساسة')}</p></div></div></section>
      </>}

      {view === 'creches' && <section className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-black text-slate-950">{text('Toutes les crèches', 'كل الروضات')}</h2><p className="mt-1 text-sm text-slate-500">{text('Suivez les abonnements, l’activité et les prolongations depuis une seule liste.', 'تابعوا الاشتراكات والنشاط والتمديدات من قائمة واحدة.')}</p></div><div className="flex w-full gap-2 sm:w-auto"><label className="relative block min-w-0 flex-1 sm:w-80"><Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder={text('Rechercher une crèche...', 'البحث عن روضة...')} className="w-full rounded-xl border border-slate-200 bg-white py-2.5 ps-9 pe-3 text-xs font-semibold outline-none transition focus:border-indigo-500" /></label><button type="button" onClick={exportCrèches} title={text('Exporter la liste filtrée', 'تصدير القائمة المفلترة')} className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2.5 text-xs font-black text-indigo-700 transition hover:bg-indigo-50"><Download className="h-4 w-4" /><span className="hidden sm:inline">CSV</span></button></div></div><div className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm"><div className="mobile-scroll-x"><table className="min-w-[920px] w-full text-start"><thead><tr className="border-b border-slate-100 bg-slate-50 text-[10px] font-black uppercase tracking-wider text-slate-400"><th className="px-5 py-4">{text('Crèche / Directrice', 'الروضة / المديرة')}</th><th className="px-5 py-4">{text('Statut', 'الحالة')}</th><th className="px-5 py-4">{text('Échéance', 'تاريخ الانتهاء')}</th><th className="px-5 py-4">{text('Activité', 'النشاط')}</th><th className="px-5 py-4 text-center">{text('Actions', 'إجراءات')}</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredDirectors.map(account => { const remaining = daysUntil(account.dateFinAbonnement); const isExpiring = account.abonnementActif && (remaining ?? 999) <= 7; const busy = busyId?.endsWith(account.id); return <tr key={account.id} className="hover:bg-slate-50/70"><td className="px-5 py-4"><button type="button" onClick={() => setSelectedDirectorId(account.id)} className="text-start"><p className="font-black text-slate-900 hover:text-indigo-700">{accountLabel(account)}</p><p className="mt-1 text-xs font-semibold text-slate-500">{account.prenom} {account.nom} · {account.email}</p></button></td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ${account.abonnementActif ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{account.abonnementActif ? text('Actif', 'نشط') : text('Suspendu', 'معلق')}</span>{account.approvalStatus === 'pending' && <span className="ms-1 inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-black text-amber-700">{text('À valider', 'بانتظار الموافقة')}</span>}</td><td className="px-5 py-4"><p className={`text-xs font-black ${isExpiring ? 'text-rose-600' : 'text-slate-700'}`}>{formatDate(account.dateFinAbonnement)}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{remaining === null ? text('Non renseignée', 'غير محدد') : remaining < 0 ? 'Expiré' : `${remaining} ${text('jour(s)', 'يوم')}`}</p></td><td className="px-5 py-4"><span className={`text-xs font-bold ${isStale(account.lastActivityAt) ? 'text-amber-700' : 'text-emerald-700'}`}>{isStale(account.lastActivityAt) ? text('À relancer', 'تحتاج متابعة') : text('Récente', 'حديثة')}</span><p className="mt-1 text-[10px] text-slate-400">{formatDateTime(account.lastActivityAt)}</p></td><td className="px-5 py-4"><div className="flex justify-center gap-1.5"><button type="button" onClick={() => setSelectedDirectorId(account.id)} title={text('Ouvrir la fiche', 'فتح الملف')} className="rounded-xl border border-slate-200 p-2 text-slate-600 transition hover:bg-slate-50"><ChevronRight className="h-4 w-4 rtl:rotate-180" /></button><button type="button" disabled={Boolean(busy)} onClick={() => void extendTrial(account)} title={text('Prolonger l’essai de 15 jours', 'تمديد التجربة 15 يوماً')} className="rounded-xl border border-indigo-200 p-2 text-indigo-600 transition hover:bg-indigo-50 disabled:opacity-50"><CalendarClock className="h-4 w-4" /></button><button type="button" disabled={Boolean(busy)} onClick={() => void toggleSubscription(account)} title={account.abonnementActif ? text('Suspendre', 'تعليق') : text('Réactiver', 'إعادة تفعيل')} className={`rounded-xl border p-2 transition disabled:opacity-50 ${account.abonnementActif ? 'border-rose-200 text-rose-600 hover:bg-rose-50' : 'border-emerald-200 text-emerald-600 hover:bg-emerald-50'}`}>{account.abonnementActif ? <CirclePause className="h-4 w-4" /> : <PlayCircle className="h-4 w-4" />}</button></div></td></tr>; })}{filteredDirectors.length === 0 && <tr><td colSpan={5} className="px-5 py-14 text-center text-sm font-bold text-slate-400">{text('Aucune crèche trouvée.', 'لم يتم العثور على روضة.')}</td></tr>}</tbody></table></div></div>{selectedDirector && <AdminCrecheDetail account={selectedDirector} onClose={() => setSelectedDirectorId(null)} onNavigate={onNavigate} />}</section>}
      ...
      {view === 'historique' && <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-black text-slate-950">{text('Historique Administrateur', 'سجل الإدارة')}</h2><p className="mt-1 text-sm text-slate-500">{text('Les actions sensibles de ce centre sont conservées pour le suivi interne.', 'يتم حفظ الإجراءات الحساسة في هذا المركز للمتابعة الداخلية.')}</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">{filteredAuditLogs.length} {text('action(s)', 'إجراء')}</span><button type="button" onClick={exportHistorique} title={text('Exporter l’historique filtré', 'تصدير السجل المفلتر')} className="rounded-xl border border-indigo-200 p-2 text-indigo-700 transition hover:bg-indigo-50"><Download className="h-4 w-4" /></button></div></div><div className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_13rem]"><label className="relative block"><Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={historySearch} onChange={event => setHistorySearch(event.target.value)} placeholder={text('Rechercher une cible ou une action...', 'البحث عن هدف أو إجراء...')} className="w-full rounded-xl border border-slate-200 bg-white py-2.5 ps-9 pe-3 text-xs font-semibold outline-none focus:border-indigo-500" /></label><select value={historyAction} onChange={event => setHistoryAction(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-bold outline-none focus:border-indigo-500"><option value="all">{text('Toutes les actions', 'كل الإجراءات')}</option>{Array.from(new Set<string>(adminAuditLogs.map(entry => String(entry.action)))).sort().map(action => <option key={action} value={action}>{action.replaceAll('_', ' ')}</option>)}</select></div><div className="mt-5 space-y-2">{filteredAuditLogs.slice(0, 50).map(entry => <article key={entry.id} className="flex items-start gap-3 rounded-2xl border border-slate-100 bg-slate-50/70 p-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-indigo-600 shadow-xs"><History className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-sm font-black text-slate-800">{entry.targetLabel || entry.targetId}</p><p className="mt-0.5 text-xs font-semibold text-slate-500">{entry.action.replaceAll('_', ' ')} · {formatDateTime(entry.createdAt)}</p></div></article>)}{filteredAuditLogs.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 px-5 py-14 text-center text-sm font-bold text-slate-400">{text('Aucune action ne correspond aux filtres.', 'لا توجد إجراءات مطابقة للفلاتر.')}</div>}</div></section>}
    </div>
  );
}
