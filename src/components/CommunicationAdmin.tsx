      ...
      {section === 'messages' ? (
        ...
        <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
          ...
          <div className="p-5 space-y-4">{orderedSignalements.length === 0 ? <div className="py-16 text-center text-slate-400"><FileText className="w-12 h-12 mx-auto mb-3 text-slate-200" /><p className="font-bold">{isFrench ? 'Aucun signalement ou suggestion.' : 'لا توجد بلاغات أو اقتراحات.'}</p></div> : orderedSignalements.map(item => {
            ...
            return <article key={item.id} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 md:p-5"><div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="px-2.5 py-1 rounded-lg bg-indigo-100 text-indigo-700 text-[10px] font-black">{typeLabel(item.type)}</span><span className="text-[10px] text-slate-400">{new Date(item.date).toLocaleString(isFrench ? 'fr-FR' : 'ar-DZ', { dateStyle: 'short', timeStyle: 'short' })}</span></div><h3 className="text-base font-black text-slate-900 mt-2">{item.titre}</h3><p className="text-xs text-slate-500 mt-1">{item.userName} · {item.nomCreche}</p></div><div className="flex items-center gap-2 shrink-0"><select value={item.statut} onChange={event => void handleStatusChange(item.id, event.target.value as SignalementStatut)} className={`px-3 py-2 rounded-xl border text-xs font-black outline-none ${item.statut === 'resolu' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : item.statut === 'rejete' ? 'bg-rose-50 border-rose-200 text-rose-700' : item.statut === 'en_cours' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-indigo-50 border-indigo-200 text-indigo-700'}`}>{signalementStatuses.map(status => <option key={status} value={status}>{statusLabel(status)}</option>)}</select><button type="button" onClick={() => void handleDeleteSignalement(item)} className="p-2 rounded-xl text-rose-500 hover:bg-rose-50" title={isFrench ? 'Supprimer définitivement' : 'حذف نهائي'}><Trash2 className="w-4 h-4" /></button></div></div><p className="text-sm text-slate-700 whitespace-pre-line bg-white rounded-xl border border-slate-100 p-4 mt-4">{item.description}</p><div className="mt-4"><label className="text-xs font-black text-slate-600 flex items-center gap-2 mb-2"><MessageSquareQuote className="w-4 h-4 text-indigo-500" />{isFrench ? 'Réponse à l’utilisateur' : 'الرد على المستخدم'}</label><div className="flex flex-col sm:flex-row gap-2"><textarea value={response} onChange={event => setResponseDrafts(previous => ({ ...previous, [item.id]: event.target.value }))} rows={2} placeholder={isFrench ? 'Écrire une réponse ou une indication...' : 'اكتب رداً أو توضيحاً...'} className="flex-1 px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-xs outline-none focus:border-indigo-400 resize-none" /><button type="button" onClick={() => void handleSaveResponse(item)} disabled={!response.trim() || savingResponseId === item.id} className="sm:self-end px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-black hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2"><Save className="w-3.5 h-3.5" />{savingResponseId === item.id ? (isFrench ? 'Envoi...' : 'جارٍ الحفظ...') : (isFrench ? 'Enregistrer' : 'حفظ')}</button></div></div>{item.reponseAdmin && <div className="mt-3 rounded-xl bg-emerald-50 border border-emerald-100 p-3"><p className="text-[10px] font-black text-emerald-700">{isFrench ? 'Réponse actuellement publiée' : 'الرد المنشور حالياً'}</p><p className="text-xs text-emerald-900 mt-1 whitespace-pre-line">{item.reponseAdmin}</p></div>}</article>;
          })}</div>
        </div>
      )}
    </div>
