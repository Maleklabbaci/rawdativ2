...
export default function Community() {
  ...
  const certificationChildrenCount = Math.max(registeredChildrenCount, user?.certificationEnfants || 0);
  const isCurrentUserCertified = Boolean(user?.estCertifie) || certificationChildrenCount >= 30;
  ...
  const handleSubmitPost = async (event: FormEvent) => {
    ...
    try {
      await addCommunityPost({
        ...
        authorEstCertifie: isCurrentUserCertified,
        authorCertificationEnfants: certificationChildrenCount,
        crecheId: user.id,
    ...
    <div className="min-h-full min-w-0 overflow-x-hidden bg-slate-50 pb-14" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="mx-auto min-w-0 max-w-7xl px-3 py-4 sm:px-4 sm:py-5 md:px-6">
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-indigo-600"><Sparkles className="h-4 w-4" />Rawdha Connect</div><h1 className="mt-1 text-xl sm:text-2xl font-black tracking-tight text-slate-900 md:text-3xl">Votre réseau professionnel</h1><p className="mt-1 text-sm text-slate-500">Partagez, échangez et découvrez les crèches de la communauté.</p></div>
          <div className="flex items-center gap-2"><label className="hidden items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm md:flex"><Search className="h-4 w-4 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Rechercher" className="w-44 bg-transparent text-sm outline-none" /></label><button type="button" onClick={() => setShowComposer(true)} className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-black text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700"><Plus className="h-4 w-4" />Nouvelle publication</button></div>
        </div>

        <AnimatePresence initial={false}>{showProfileEditor && renderProfileEditor()}</AnimatePresence>
        <div className="mobile-scroll-x mb-5 flex gap-2 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm"><button type="button" onClick={() => setActiveView('feed')} className={`rounded-xl px-4 py-2.5 text-sm font-black whitespace-nowrap ${activeView === 'feed' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>Accueil</button><button type="button" onClick={() => { setActiveView('profile'); setSelectedProfileId(user.id); }} className={`rounded-xl px-4 py-2.5 text-sm font-black whitespace-nowrap ${activeView === 'profile' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>Mon profil</button><button type="button" onClick={() => setActiveView('reposts')} className={`rounded-xl px-4 py-2.5 text-sm font-black whitespace-nowrap ${activeView === 'reposts' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>Mes republications</button><div className="ml-auto hidden items-center gap-2 px-3 text-xs font-semibold text-slate-400 lg:flex"><Users className="h-4 w-4" />{profiles.length} profil{profiles.length > 1 ? 's' : ''} visible{profiles.length > 1 ? 's' : ''}</div></div>

        {activeView === 'profile' && selectedProfile ? renderPublicProfile(selectedProfile) : (
        <div className="grid min-w-0 gap-5 lg:grid-cols-[260px_minmax(0,1fr)_280px]">
