    ...
    if (activeStep === 'repas') {
      return (
        <div className="space-y-5">
          ...
          {todayMeals.length === 0 ? <EmptyState icon={Utensils} text={text('Aucun repas planifié pour aujourd’hui. Vous pouvez confirmer et passer cette étape.', 'لا توجد وجبات مبرمجة لليوم. يمكنك التأكيد وتجاوز هذه المرحلة.')} /> : <div className="grid gap-3 md:grid-cols-2">{todayMeals.map((meal) => <div key={meal.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-orange-600">{meal.type === 'Déjeuner' ? text('Déjeuner', 'الغداء') : text('Goûter', 'اللمجة')}</p><p className="mt-1 text-base font-black text-slate-900">{meal.menu}</p></div><Utensils className="h-5 w-5 text-orange-500" /></div><p className="mt-3 text-xs font-semibold text-slate-400">{(meal as Repas & { allergenes?: string }).allergenes || text('Allergènes à vérifier dans la fiche repas', 'تحقق من الحساسية في بطاقة الوجبة')}</p></div>)}</div>}
