  ...
  return (
    <div className="fixed inset-0 z-[120] isolate overflow-hidden overscroll-none bg-slate-950/70 backdrop-blur-sm" dir={isArabic ? 'rtl' : 'ltr'}>
      <div className="flex h-[100dvh] max-h-[100dvh] min-h-0 w-full min-w-0 flex-col overflow-hidden bg-slate-50 text-slate-900">
        ...
        <div className="shrink-0 border-b border-slate-200 bg-white px-2.5 py-2 lg:hidden"><div className="flex min-w-0 snap-x snap-mandatory gap-1.5 overflow-x-auto overscroll-contain pb-1 scrollbar-none">{steps.map((step, index) => { const Icon = step.icon; const isActive = step.key === activeStep; const isDone = completedSteps.includes(step.key) || (step.key === 'presences' && isDayValidated); return <button type="button" key={step.key} onClick={() => setActiveStep(step.key)} className={`flex min-w-[116px] max-w-[150px] snap-start items-start gap-1.5 rounded-xl px-2 py-1.5 text-left transition ${isActive ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-50 text-slate-500'}`}><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg ${isActive ? 'bg-white/15' : 'bg-white'}`}>{isDone ? <Check className="h-3 w-3" /> : <Icon className="h-3 w-3" />}</span><span className="min-w-0 whitespace-normal"><span className="block break-words text-[9px] font-black leading-tight">{index + 1}. {isArabic ? step.ar : step.fr}</span><span className={`block break-words text-[8px] font-semibold leading-tight ${isActive ? 'text-indigo-100' : 'text-slate-400'}`}>{isArabic ? step.descriptionAr : step.descriptionFr}</span></span></button>; })}</div></div>
