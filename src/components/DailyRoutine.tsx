...
export default function DailyRoutine({ onClose, onNavigate }: DailyRoutineProps) {
        ...
        <div className="space-y-5">
          <div className="rounded-[2rem] bg-gradient-to-br from-indigo-950 via-indigo-900 to-violet-800 p-5 text-white shadow-xl sm:p-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-indigo-200"><Zap className="h-4 w-4" /> {text('Votre journée commence ici', 'يبدأ يومك من هنا')}</div>
                <h2 className="max-w-2xl text-2xl font-black tracking-tight sm:text-4xl">{text('Une journée claire, étape par étape.', 'يوم واضح، خطوة بخطوة.')}</h2>
                <p className="mt-3 max-w-xl text-sm leading-6 text-indigo-100/80">{text('Ouvrez la crèche, préparez les équipes et lancez le suivi quotidien sans chercher dans les menus.', 'افتح الحضانة، حضّر الفريق وابدأ المتابعة اليومية دون البحث بين القوائم.')}</p>
              </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => goNext('ouverture')} className="flex items-center justify-between rounded-2xl bg-indigo-600 p-4 text-left text-white shadow-lg shadow-indigo-600/20 transition hover:bg-indigo-700 active:scale-[0.98]"><span><span className="block text-sm font-black">{text('Commencer la journée', 'ابدأ اليوم')}</span><span className="mt-1 block text-xs font-semibold text-indigo-100">{text('Passer aux présences', 'الانتقال إلى الحضور')}</span></span><ArrowRight className="h-5 w-5 rtl:rotate-180" /></button>
            <div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="flex items-center gap-2 text-sm font-black text-slate-800"><Info className="h-4 w-4 text-indigo-600" /> {text('Raccourcis de préparation', 'اختصارات التحضير')}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => openPage('classes')} className="rounded-xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100">{text('Gérer les classes', 'إدارة الأقسام')}</button><button type="button" onClick={() => openPage('personnel')} className="rounded-xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100">{text('Voir l’équipe', 'عرض الفريق')}</button></div></div>
          </div>
        </div>
      );
    }
    ...
    if (activeStep === 'sante') {
      return (
        <div className="space-y-5">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button type="button" onClick={() => openPage('presences')} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-xs font-black text-slate-600">{text('Ouvrir le registre santé', 'فتح سجل الصحة')}</button><button type="button" onClick={() => goNext('sante')} className="flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white">{text('Santé vérifiée', 'تم التحقق من الصحة')}<ArrowRight className="h-4 w-4 rtl:rotate-180" /></button></div>
        </div>
      );
    }
    ...
    if (activeStep === 'repas') {
      return (
        <div className="space-y-5">
          ...
          {todayMeals.length === 0 ? <EmptyState icon={Utensils} text={text('Aucun repas planifié pour aujourd’hui.', 'لا توجد وجبات مبرمجة لليوم.')} actionLabel={text('Planifier un repas', 'برمجة وجبة')} onAction={() => openPage('repas')} /> : <div className="grid gap-3 md:grid-cols-2">{todayMeals.map((meal) => <div key={meal.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-orange-600">{meal.type === 'Déjeuner' ? text('Déjeuner', 'الغداء') : text('Goûter', 'اللمجة')}</p><p className="mt-1 text-base font-black text-slate-900">{meal.menu}</p></div><Utensils className="h-5 w-5 text-orange-500" /></div><p className="mt-3 text-xs font-semibold text-slate-400">{(meal as Repas & { allergenes?: string }).allergenes || text('Allergènes à vérifier dans la fiche repas', 'تحقق من الحساسية في بطاقة الوجبة')}</p></div>)}</div>}
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-emerald-600"><CheckCircle2 className="h-5 w-5" /></div><div><p className="text-sm font-black text-emerald-900">{text('Service des repas confirmé', 'تم تأكيد تقديم الوجبات')}</p><p className="text-xs font-semibold text-emerald-700/80">{text('Une seule validation suffit pour l’équipe du jour.', 'يكفي تأكيد واحد لفريق اليوم.')}</p></div></div><button type="button" onClick={() => setConfirmedMeals((value) => !value)} className={`relative h-7 w-12 rounded-full transition ${confirmedMeals ? 'bg-emerald-600' : 'bg-emerald-200'}`} aria-pressed={confirmedMeals}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${confirmedMeals ? 'left-6' : 'left-1'}`} /></button></div>
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button type="button" onClick={() => openPage('repas')} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-xs font-black text-slate-600">{text('Gérer les menus', 'إدارة الوجبات')}</button><button type="button" onClick={() => goNext('repas')} className="flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white">{text('Repas confirmés', 'تم تأكيد الوجبات')}<ArrowRight className="h-4 w-4 rtl:rotate-180" /></button></div>
        </div>
      );
    }
    ...
    if (activeStep === 'activites') {
      return (
        <div className="space-y-5">
          ...
          {todayActivities.length === 0 ? <EmptyState icon={Sparkles} text={text('Aucune activité planifiée pour aujourd’hui.', 'لا توجد أنشطة مبرمجة لليوم.')} actionLabel={text('Planifier une activité', 'برمجة نشاط')} onAction={() => openPage('activites')} /> : <div className="grid gap-3 md:grid-cols-2">{todayActivities.map((activity) => <div key={activity.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-indigo-600">{activity.groupe}</p><p className="mt-1 text-base font-black text-slate-900">{activity.titre}</p></div><Sparkles className="h-5 w-5 text-indigo-500" /></div><p className="mt-3 text-xs font-semibold text-slate-500">{(activity as Activite & { heureDebut?: string; heureFin?: string; lieu?: string }).heureDebut || '—'} - {(activity as Activite & { heureDebut?: string; heureFin?: string }).heureFin || '—'} · {(activity as Activite & { lieu?: string }).lieu || text('Lieu à préciser', 'المكان غير محدد')}</p></div>)}</div>}
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-indigo-100 bg-indigo-50 p-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-indigo-600"><Play className="h-4 w-4" /></div><div><p className="text-sm font-black text-indigo-900">{text('Programme lancé', 'تم بدء البرنامج')}</p><p className="text-xs font-semibold text-indigo-700/80">{text('Les éducateurs peuvent suivre les activités prévues.', 'يمكن للمربين متابعة الأنشطة المبرمجة.')}</p></div></div><button type="button" onClick={() => setConfirmedActivities((value) => !value)} className={`relative h-7 w-12 rounded-full transition ${confirmedActivities ? 'bg-indigo-600' : 'bg-indigo-200'}`} aria-pressed={confirmedActivities}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${confirmedActivities ? 'left-6' : 'left-1'}`} /></button></div>
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button type="button" onClick={() => openPage('activites')} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-xs font-black text-slate-600">{text('Gérer les activités', 'إدارة الأنشطة')}</button><button type="button" onClick={() => goNext('activites')} className="flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white">{text('Activités lancées', 'تم بدء الأنشطة')}<ArrowRight className="h-4 w-4 rtl:rotate-180" /></button></div>
        </div>
      );
    }
    ...
    if (activeStep === 'departs') {
      return (
        <div className="space-y-5">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button type="button" onClick={() => openPage('enfants')} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-xs font-black text-slate-600">{text('Voir les contacts enfants', 'عرض جهات اتصال الأطفال')}</button><button type="button" disabled={pendingDepartures.length > 0} onClick={() => goNext('departs')} className="flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50">{text('Tous les départs sont vérifiés', 'تم التحقق من كل المغادرات')}<ArrowRight className="h-4 w-4 rtl:rotate-180" /></button></div>
        </div>
      );
    }
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-slate-950/70 backdrop-blur-sm" dir={isArabic ? 'rtl' : 'ltr'}>
      <div className="flex h-[100dvh] w-full flex-col overflow-hidden bg-slate-50 text-slate-900">
        <header className="shrink-0 border-b border-slate-200 bg-white/95 px-3 py-3 shadow-sm backdrop-blur sm:px-6 sm:py-4">
          <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-600/20"><Zap className="h-5 w-5" /></div><div className="min-w-0"><p className="truncate text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600">RAWDHA+ • {text('Mode journée', 'وضع اليوم')}</p><h1 className="truncate text-base font-black sm:text-xl">{text('Démarrer ma journée', 'ابدأ يومي')}</h1></div></div><div className="flex items-center gap-2"><span className="hidden rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-500 sm:block">{dateLabel}</span><button type="button" onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50" aria-label={text('Fermer', 'إغلاق')}><X className="h-5 w-5" /></button></div></div>
        </header>
        <div className="shrink-0 border-b border-slate-200 bg-white px-3 py-2 lg:hidden"><div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">{steps.map((step, index) => { const Icon = step.icon; const isActive = step.key === activeStep; const isDone = completedSteps.includes(step.key) || (step.key === 'presences' && isDayValidated); return <button type="button" key={step.key} onClick={() => setActiveStep(step.key)} className={`flex min-w-[112px] items-center gap-2 rounded-xl px-3 py-2 text-left transition ${isActive ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-50 text-slate-500'}`}><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${isActive ? 'bg-white/15' : 'bg-white'}`}>{isDone ? <Check className="h-3.5 w-3.5" /> : <Icon className="h-3.5 w-3.5" />}</span><span><span className="block text-[10px] font-black">{index + 1}. {isArabic ? step.ar : step.fr}</span><span className={`block text-[9px] font-semibold ${isActive ? 'text-indigo-100' : 'text-slate-400'}`}>{isArabic ? step.descriptionAr : step.descriptionFr}</span></span></button>; })}</div></div>
        <div className="mx-auto flex min-h-0 w-full max-w-[1500px] flex-1">
          ...
          <main className="min-w-0 flex-1 overflow-y-auto"><div className="mx-auto max-w-5xl p-3 pb-8 sm:p-6 sm:pb-12 lg:p-10"><div className="mb-5 flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-indigo-600">{text(`Étape ${activeStepIndex + 1} sur ${steps.length}`, `المرحلة ${activeStepIndex + 1} من ${steps.length}`)}</p><h2 className="mt-1 text-xl font-black text-slate-900 sm:text-2xl">{isArabic ? activeStepData.ar : activeStepData.fr}</h2></div><div className="hidden items-center gap-2 text-xs font-bold text-slate-400 sm:flex"><span>{text('Aujourd’hui', 'اليوم')}</span><ChevronRight className="h-4 w-4 rtl:rotate-180" /><span className="text-slate-700">{dateLabel}</span></div></div>{renderStepContent()}</div></main>
        </div>
        <footer className="shrink-0 border-t border-slate-200 bg-white px-3 py-2 sm:px-6"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3"><button type="button" disabled={activeStepIndex === 0} onClick={() => setActiveStep(steps[activeStepIndex - 1]?.key || 'ouverture')} className="flex items-center gap-1 rounded-xl px-3 py-2 text-xs font-black text-slate-500 hover:bg-slate-50 disabled:invisible"><ChevronLeft className="h-4 w-4 rtl:rotate-180" />{text('Précédent', 'السابق')}</button><div className="hidden items-center gap-1.5 sm:flex">{steps.map((step) => <span key={step.key} className={`h-1.5 rounded-full transition-all ${step.key === activeStep ? 'w-8 bg-indigo-600' : completedSteps.includes(step.key) ? 'w-3 bg-emerald-500' : 'w-3 bg-slate-200'}`} />)}</div><button type="button" onClick={() => activeStepIndex === steps.length - 1 ? void completeClosing() : goNext(activeStep)} className="flex items-center gap-1 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-black text-white hover:bg-indigo-700"><span>{activeStepIndex === steps.length - 1 ? text('Fermer la journée', 'إغلاق اليوم') : text('Suivant', 'التالي')}</span>{activeStepIndex === steps.length - 1 ? <Check className="h-4 w-4" /> : <ChevronRight className="h-4 w-4 rtl:rotate-180" />}</button></div></footer>
      </div>
    </div>
  );
}
...
function SectionIntro({ icon: Icon, title, description }: { icon: typeof DoorClosed; title: string; description: string }) {
  return <div className="flex items-start gap-3 rounded-3xl border border-slate-200 bg-white p-5 shadow-xs sm:p-6"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600"><Icon className="h-5 w-5" /></div><div><h2 className="text-xl font-black text-slate-900">{title}</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">{description}</p></div></div>;
}
