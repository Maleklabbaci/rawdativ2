 ...
 * grands aplats bleu, jaune, menthe et corail, cadres capsules et photos humaines complètes.
 * Les personnes sont toujours cadrées avec marge ; les mouvements restent courts et accessibles.
 */
import {
  ArrowLeft,
  ArrowRight,
...
const heroImage = '/manus-storage/rawdha-full-hero_ceecefd4.jpg';
const activityImage = '/manus-storage/rawdha-full-child_46c13f49.jpg';
const teamImage = '/manus-storage/rawdha-full-directors_4f617609.jpg';
      ...
      <main>
        <section id="plateforme" className="relative overflow-hidden bg-[#0876c9] px-4 pb-0 pt-28 text-white sm:px-6 sm:pt-36 lg:px-8">
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl gap-12 pb-14 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:gap-16 lg:pb-20">
            ...
            <div data-reveal className="rawdah-reveal rawdah-delay-2 relative min-h-[410px] sm:min-h-[500px]"><div className="absolute right-[6%] top-2 h-40 w-40 rounded-full bg-[#f9b928] sm:h-52 sm:w-52" /><div className="absolute left-[5%] top-[20%] h-36 w-36 rounded-full bg-[#00a66c]" /><div className="absolute right-0 top-8 w-[94%] rounded-[56px] border-[7px] border-white bg-[#fffaf0] p-3 shadow-[0_18px_0_rgba(16,41,58,0.20)] sm:rounded-[72px] sm:p-4"><div className="aspect-[3/2] overflow-hidden rounded-[42px] bg-[#fff4d2]"><img src={heroImage} alt={content.heroPhotoAlt} className="h-full w-full object-contain p-1" /></div></div><div className="absolute bottom-0 left-0 w-[88%] rounded-[32px] border-2 border-[#10293a]/15 bg-white p-4 text-[#10293a] shadow-[0_13px_0_rgba(16,41,58,0.24)] sm:rounded-[38px] sm:p-5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><img src="/rawdah-logo.png" alt="" className="h-8 w-8 rounded-full object-contain" /><p className="text-xs font-black">RAWDHA+</p></div><span className="rounded-full bg-[#00a66c] px-2.5 py-1 text-[10px] font-black text-white">{content.heroCardLabel}</span></div><div className="mt-4 grid gap-2 sm:grid-cols-3">{content.heroCardItems.map((item, index) => <div key={item} className="flex items-center gap-2 rounded-2xl bg-[#fff4d2] px-2.5 py-2 text-[10px] font-black leading-4"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-white text-[#0876c9]">{index === 0 ? <CalendarCheck className="h-3 w-3" /> : index === 1 ? <ClipboardList className="h-3 w-3" /> : <Bell className="h-3 w-3" />}</span>{item}</div>)}</div></div></div>
          </div>
