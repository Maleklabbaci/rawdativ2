/**
 * Style context — Rawdha+ « Cadre vivant » : vitrine de crèche stable, avec personnages PNG détourés.
 * Les enfants et l’éducatrice ont leur propre zone de sécurité et des mouvements uniquement par transform.
 */
import {
  ArrowLeft,
  ArrowRight,
...
const heroChildCutout = '/manus-storage/rawdha-child-hero-cutout_535b6eea.png';
const educatorCutout = '/manus-storage/rawdha-educator-cutout_2e251274.png';
const childrenConnectCutout = '/manus-storage/rawdha-children-connect-cutout_973a2c26.png';
...
export default function LandingPage({ onNavigate }: LandingPageProps) {
  ...
  return (
    <div dir={isArabic ? 'rtl' : 'ltr'} className="min-h-screen overflow-x-hidden bg-[#f8fafc] text-[#10293a]">
      ...
      <main>
        <section id="plateforme" className="relative overflow-hidden border-b border-slate-200 bg-white px-4 py-14 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        ...
        <section id="connect" className="scroll-mt-20 bg-[#eef6fc] px-4 py-14 sm:px-6 sm:py-20 lg:px-8"><div className="mx-auto grid max-w-7xl gap-10 rounded-2xl border border-blue-100 bg-white p-6 sm:p-10 lg:grid-cols-[1fr_0.78fr] lg:items-center lg:p-12"><div><div className="inline-flex items-center gap-2 text-sm font-bold text-[#0876c9]"><Network className="h-4 w-4" />{content.connectLabel}</div><h2 className="mt-4 text-3xl font-black tracking-tight text-[#10293a] sm:text-4xl">{content.connectTitle}</h2><p className="mt-4 max-w-2xl text-base font-medium leading-7 text-slate-600">{content.connectText}</p><button type="button" onClick={() => onNavigate('login')} className="mt-7 inline-flex items-center gap-2 rounded-lg bg-[#0876c9] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#0666ae]">{content.connectAction}<Arrow className="h-4 w-4" /></button></div><div className="relative min-h-[280px] rounded-xl border border-slate-200 bg-slate-50 p-5 sm:min-h-[300px] lg:pr-44"><p className="relative z-10 text-sm font-black text-[#10293a]">Rawdha Connect</p><div className="relative z-10 mt-5 space-y-3">{content.connectItems.map((item, index) => <div key={item} className="flex items-center gap-3 rounded-lg bg-white p-3 shadow-sm"><span className="grid h-8 w-8 place-items-center rounded-lg bg-blue-50 text-[#0876c9]">{index === 0 ? <Network className="h-4 w-4" /> : index === 1 ? <MessageCircle className="h-4 w-4" /> : <Bell className="h-4 w-4" />}</span><span className="text-sm font-bold text-slate-700">{item}</span></div>)}</div><span className="absolute bottom-4 right-4 h-24 w-24 rounded-full bg-amber-50" /><img src={childrenConnectCutout} alt="" className="rawdah-child-float pointer-events-none absolute bottom-0 right-0 z-10 h-52 w-auto object-contain sm:h-60" /></div></div></section>
