      ...
      <header className={`fixed inset-x-0 top-0 z-50 px-3 py-3 transition-all duration-300 sm:px-6 ${scrolled ? 'bg-[#fffaf0]/85 shadow-[0_3px_18px_rgba(16,41,58,0.12)] backdrop-blur' : ''}`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-full border border-[#10293a]/15 bg-white px-3 py-2 shadow-[0_7px_0_rgba(16,41,58,0.08)] sm:px-4">
          ...
          <div className="flex items-center gap-1.5 md:gap-2">
            <div className="hidden rounded-full bg-[#edf1ed] p-1 md:flex"><button type="button" onClick={() => setLanguage('fr')} aria-pressed={!isArabic} className={`rounded-full px-2.5 py-1 text-[10px] font-black ${!isArabic ? 'bg-[#10293a] text-white' : 'text-[#10293a]'}`}>FR</button><button type="button" onClick={() => setLanguage('ar')} aria-pressed={isArabic} className={`rounded-full px-2.5 py-1 text-[10px] font-black ${isArabic ? 'bg-[#10293a] text-white' : 'text-[#10293a]'}`}>ع</button></div>
            <div className="flex rounded-full bg-[#edf1ed] p-1 md:hidden"><button type="button" onClick={() => setLanguage('fr')} aria-pressed={!isArabic} className={`grid h-7 w-7 place-items-center rounded-full text-[10px] font-black ${!isArabic ? 'bg-[#10293a] text-white' : 'text-[#10293a]'}`}>FR</button><button type="button" onClick={() => setLanguage('ar')} aria-pressed={isArabic} className={`grid h-7 w-7 place-items-center rounded-full text-xs font-black ${isArabic ? 'bg-[#10293a] text-white' : 'text-[#10293a]'}`}>ع</button></div>
            ...
            <button type="button" onClick={() => onNavigate('request')} className="hidden items-center gap-1.5 rounded-full bg-[#f9b928] px-4 py-2.5 text-xs font-black text-[#10293a] shadow-[0_3px_0_#d39108] transition hover:-translate-y-0.5 active:translate-y-0 active:shadow-none md:inline-flex">{content.nav.request}<Arrow className="h-3.5 w-3.5" /></button>
          </div>
      ...
      <main>
        <section id="plateforme" className="relative overflow-hidden bg-[#0876c9] px-4 pb-0 pt-28 text-white sm:px-6 sm:pt-36 lg:px-8">
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl gap-12 pb-14 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:gap-16 lg:pb-20">
            ...
            <div data-reveal className="rawdah-reveal rawdah-delay-2 relative min-h-[410px] sm:min-h-[500px]"><div className="absolute right-[6%] top-2 h-40 w-40 rounded-full bg-[#f9b928] sm:h-52 sm:w-52" /><div className="absolute left-[5%] top-[20%] h-36 w-36 rounded-full bg-[#00a66c]" /><div className="absolute right-0 top-8 w-[94%] rounded-[56px] border-[7px] border-white bg-[#fffaf0] p-4 shadow-[0_18px_0_rgba(16,41,58,0.20)] sm:rounded-[72px] sm:p-6"><div className="rounded-[38px] bg-[#fff4d2] p-5 text-[#10293a] sm:p-7"><div className="flex items-center justify-between gap-3"><span className="grid h-16 w-16 place-items-center rounded-full bg-[#0876c9] shadow-[0_4px_0_#075b9a]"><img src="/rawdah-logo.png" alt="Rawdha+" className="h-11 w-11 rounded-full object-contain" /></span><span className="rounded-full bg-[#00a66c] px-3 py-1.5 text-[10px] font-black text-white">{content.heroCardLabel}</span></div><p className="rawdah-display mt-8 max-w-xs text-3xl font-black leading-none sm:text-4xl">{isArabic ? 'كل ما تحتاجه حضانتك، في مكان واحد.' : 'Votre crèche, claire et organisée.'}</p><div className="mt-7 grid gap-2 sm:grid-cols-3">{content.heroCardItems.map((item, index) => <div key={item} className="flex items-center gap-2 rounded-2xl bg-white px-2.5 py-2.5 text-[10px] font-black leading-4 shadow-sm"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#d6eff0] text-[#0876c9]">{index === 0 ? <CalendarCheck className="h-3 w-3" /> : index === 1 ? <ClipboardList className="h-3 w-3" /> : <Bell className="h-3 w-3" />}</span>{item}</div>)}</div></div></div></div>
          </div>
