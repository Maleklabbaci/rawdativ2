...
export default function LanguageChoiceModal({ onChoose }: LanguageChoiceModalProps) {
  return (
    <div className="fixed inset-0 z-[130] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-md" dir="ltr" role="dialog" aria-modal="true" aria-labelledby="language-choice-title">
      <motion.div
        initial={{ opacity: 0, y: 24, scale: 0.96 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ type: 'spring', stiffness: 260, damping: 24 }}
        className="w-full max-w-2xl overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-indigo-950/40"
      >
        <div className="relative overflow-hidden bg-gradient-to-br from-indigo-700 via-violet-600 to-fuchsia-600 px-6 py-8 text-center text-white sm:px-10 sm:py-10">
          ...
          <h1 id="language-choice-title" className="relative mt-4 text-2xl font-black tracking-tight sm:text-3xl">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-8">
          <button
            type="button"
            onClick={() => onChoose('fr')}
            className="group rounded-2xl border-2 border-slate-100 bg-slate-50 p-6 text-left transition hover:border-indigo-500 hover:bg-indigo-50/50 hover:shadow-lg hover:shadow-indigo-100 cursor-pointer"
          >
          ...
          <button
            ...
            onClick={() => onChoose('ar')}
            dir="rtl"
            className="group rounded-2xl border-2 border-slate-100 bg-slate-50 p-6 text-right transition hover:border-violet-500 hover:bg-violet-50/50 hover:shadow-lg hover:shadow-violet-100 cursor-pointer"
          >
            <div className="flex items-center justify-between">
